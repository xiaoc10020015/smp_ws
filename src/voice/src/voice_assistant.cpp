...
{
	int         ret                  = MSP_SUCCESS;
	const char* login_params         = "appid = 52b7f17b, work_dir = .";//登录参数,appid与msc库绑定,请勿随意改动
	/*
	* rdn:           合成音频数字发音方式
	* volume:        合成音频的音量
	* pitch:         合成音频的音调
	...
	*/
    ...
    auto node = std::make_shared<rclcpp::Node>("TextToSpeech");
	rclcpp::Subscription<std_msgs::msg::String>::SharedPtr wakeUpSub = node->create_subscription<std_msgs::msg::String>("voiceWords", 1000, voiceWordsCallback);
    rclcpp::spin(node);
	rclcpp::shutdown();
	return 0;
}
