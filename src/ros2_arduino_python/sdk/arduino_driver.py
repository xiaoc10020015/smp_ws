...
class Arduino:
    ...
    def __init__(self, node, port="/dev/ttyUSB0", baudrate=57600, timeout=0.5, motors_reversed=False):
    ...
    def connect(self):
        try:
            ...
            print(f"Serial Exception:")
            print(sys.exc_info())
            print(f"Traceback follows:")
            traceback.print_exc(file=sys.stdout)
            print(f"Cannot connect to Arduino!")
            os._exit(1)
    ...
    def execute_ack(self, cmd):
        ...
        try:
            ...
            self.mutex.release()
            print(f"execute_ack exception when executing {cmd}")
            print(sys.exc_info())
            return 0
    ...
    def update_pid(self, Kp, Kd, Ki, Ko):
        ...
        '''
        print(f"Updating PID parameters")
    ...
    def get_encoder_counts(self):
        ...
        if len(values) != 2:
            print(f"Encoder count was not 2")
            raise SerialException
            return None
        else:
...
if __name__ == "__main__":
    ...
    print(f"Reading on analog port 0 {myArduino.analog_read(0)}")
    print(f"Reading on digital port 0 {myArduino.digital_read(0)}")
    print(f"Blinking the LED 3 times")
    for i in range(3):
        myArduino.digital_write(13, 1)
        time.sleep(1.0)
    #print(f"Current encoder counts", myArduino.encoders()
